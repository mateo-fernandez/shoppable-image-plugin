...
import {createShoppableElement} from './shoppable_element';
...
const createShoppableImage = (parent: Element, config: ConfigType) => {
    ...
    for (const element of config.elements) {
        createShoppableElement(parent, element);
    }
}
...
divShoppables.forEach(
    divShoppable => createShoppableImage(divShoppable, config)
);
